"""V11 runner checks use synthetic inputs only; never replay the frozen V11 bank."""
import json
import sys
...
from scripts import run_infinito3_trajectory_holdout_v11 as runner
...
def test_v11_preflight_accepts_frozen_manifest():
    manifest = runner.preflight()
    assert manifest["evaluation_version"] == "V11"
    assert manifest["suite_git_blob_sha1"] == runner.SUITE_BLOB_SHA1
    assert manifest["config"] == runner.CONFIG


def test_v11_default_command_is_preflight_only(monkeypatch):
    monkeypatch.setattr(runner, "preflight", lambda: {"synthetic": True})
    monkeypatch.setattr(
        runner, "TrajectoryEvaluationHarness",
        lambda *a, **k: pytest.fail("No V11 replay permitted"),
    )
    assert runner.main([]) == 0


def test_v11_preflight_rejects_modified_frozen_blob(tmp_path, monkeypatch):
    (tmp_path / "docs").mkdir()
    ...
    (tmp_path / "docs/INFINITO_3_V11_RUNNER_MANIFEST.json").write_text(
...
def test_v11_live_mode_requires_exact_revision(monkeypatch):
    ...
    with pytest.raises(SystemExit, match="Exact --expected-revision"):
        runner.main(["--authorize-live-v11", "--expected-revision", "other"])


def test_v11_main_writes_synthetic_evidence_without_real_bank(tmp_path, monkeypatch):
    scenario = TrajectoryScenario(name="synthetic_v11", steps=(TrajectoryStep(
        "Calculate 6 * 7", label="synthetic v11",
        tags=("empty_context",),
        expectation=EvaluationExpectation(answer_contains=("42",)),
    ),))
    monkeypatch.setitem(sys.modules, "src.infinito3.trajectory_holdout_v11_cases", SimpleNamespace(
        independent_trajectory_holdout_v11_suite=lambda: (scenario,)
    ))
    ...
    output = tmp_path / "v11-result"
    assert runner.main([
        "--authorize-live-v11", "--expected-revision", "synthetic-sha",
        "--output-dir", str(output),
    ]) == 0
    report = json.loads((output / "report.json").read_text())
    assert report["metadata"]["evaluation_version"] == "V11"
    assert report["metadata"]["first_v11_execution"] is True
    assert report["metadata"]["post_v10_diagnostic_holdout"] is True
    review = json.loads((output / "audit-review.json").read_text())
    assert review["integration_status"] == "NOT_APPROVED"
    assert review["requires_manual_planner_review"] is True
    assert review["requires_manual_literal_fastpath_review"] is True
    assert review["requires_manual_current_state_review"] is True
