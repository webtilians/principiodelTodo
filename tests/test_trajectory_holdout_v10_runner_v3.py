"""V10 runner checks use synthetic inputs only; never replay the frozen V10 bank."""
import json
import sys
...
from scripts import run_infinito3_trajectory_holdout_v10 as runner
...
def test_v10_preflight_accepts_frozen_manifest():
    manifest = runner.preflight()
    assert manifest["evaluation_version"] == "V10"
    assert manifest["suite_git_blob_sha1"] == runner.SUITE_BLOB_SHA1
    assert manifest["config"] == runner.CONFIG


def test_v10_default_command_is_preflight_only(monkeypatch):
    monkeypatch.setattr(runner, "preflight", lambda: {"synthetic": True})
    monkeypatch.setattr(
        runner, "TrajectoryEvaluationHarness",
        lambda *a, **k: pytest.fail("No V10 replay permitted"),
    )
    assert runner.main([]) == 0


def test_v10_preflight_rejects_modified_frozen_blob(tmp_path, monkeypatch):
    (tmp_path / "docs").mkdir()
    ...
    (tmp_path / "docs/INFINITO_3_V10_RUNNER_MANIFEST.json").write_text(
...
def test_v10_live_mode_requires_exact_revision(monkeypatch):
    ...
    with pytest.raises(SystemExit, match="Exact --expected-revision"):
        runner.main(["--authorize-live-v10", "--expected-revision", "other"])


def test_v10_main_writes_synthetic_evidence_without_real_bank(tmp_path, monkeypatch):
    scenario = TrajectoryScenario(name="synthetic_v10", steps=(TrajectoryStep(
        "Calculate 6 * 7", label="synthetic v10",
        tags=("empty_context",),
        expectation=EvaluationExpectation(answer_contains=("42",)),
    ),))
    monkeypatch.setitem(sys.modules, "src.infinito3.trajectory_holdout_v10_cases", SimpleNamespace(
        independent_trajectory_holdout_v10_suite=lambda: (scenario,)
    ))
    ...
    output = tmp_path / "v10-result"
    assert runner.main([
        "--authorize-live-v10", "--expected-revision", "synthetic-sha",
        "--output-dir", str(output),
    ]) == 0
    report = json.loads((output / "report.json").read_text())
    assert report["metadata"]["evaluation_version"] == "V10"
    assert report["metadata"]["first_v10_execution"] is True
    assert report["metadata"]["post_v9_diagnostic_holdout"] is True
