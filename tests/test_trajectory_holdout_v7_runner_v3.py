"""V7 runner checks use synthetic inputs only; never replay the frozen V7 bank."""
import json
import sys
...
from scripts import run_infinito3_trajectory_holdout_v7 as runner
...
def test_v7_preflight_accepts_frozen_manifest():
    manifest = runner.preflight()
    assert manifest["evaluation_version"] == "V7"
    assert manifest["suite_sha256"] == runner.SUITE_SHA
    assert manifest["config"] == runner.CONFIG


def test_default_command_is_preflight_only(monkeypatch):
    monkeypatch.setattr(runner, "preflight", lambda: {"synthetic": True})
    monkeypatch.setattr(runner, "TrajectoryEvaluationHarness",
                        lambda *a, **k: pytest.fail("No V7 replay permitted"))
    assert runner.main([]) == 0


def test_preflight_rejects_modified_frozen_blob(tmp_path, monkeypatch):
    (tmp_path / "docs").mkdir()
    ...
    (tmp_path / "docs/INFINITO_3_V7_RUNNER_MANIFEST.json").write_text(
...
def test_live_mode_requires_exact_revision(monkeypatch):
    ...
    with pytest.raises(SystemExit, match="Exact --expected-revision"):
        runner.main(["--authorize-live-v7", "--expected-revision", "other"])


def test_main_writes_synthetic_v7_evidence_without_real_bank(tmp_path, monkeypatch):
    scenario = TrajectoryScenario(name="synthetic_v7", steps=(TrajectoryStep(
        "Calculate 6 * 7", label="synthetic v7",
        tags=("empty_context",),
        expectation=EvaluationExpectation(answer_contains=("42",)),
    ),))
    monkeypatch.setitem(sys.modules, "src.infinito3.trajectory_holdout_v7_cases", SimpleNamespace(
        independent_trajectory_holdout_v7_suite=lambda: (scenario,)
    ))
    ...
    output = tmp_path / "v7-result"
    assert runner.main([
        "--authorize-live-v7", "--expected-revision", "synthetic-sha",
        "--output-dir", str(output),
    ]) == 0
    report = json.loads((output / "report.json").read_text())
    assert report["metadata"]["evaluation_version"] == "V7"
    assert report["metadata"]["first_v7_execution"] is True
    assert report["metadata"]["post_v6_diagnostic_holdout"] is True
