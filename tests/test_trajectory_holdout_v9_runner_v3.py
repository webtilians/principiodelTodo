"""V9 runner checks use synthetic inputs only; never replay the frozen V9 bank."""
import json
import sys
...
from scripts import run_infinito3_trajectory_holdout_v9 as runner
...
def test_v9_preflight_accepts_frozen_manifest():
    manifest = runner.preflight()
    assert manifest["evaluation_version"] == "V9"
    assert manifest["suite_git_blob_sha1"] == runner.SUITE_BLOB_SHA1
    assert manifest["config"] == runner.CONFIG


def test_v9_default_command_is_preflight_only(monkeypatch):
    monkeypatch.setattr(runner, "preflight", lambda: {"synthetic": True})
    monkeypatch.setattr(runner, "TrajectoryEvaluationHarness",
                        lambda *a, **k: pytest.fail("No V9 replay permitted"))
    assert runner.main([]) == 0


def test_v9_preflight_rejects_modified_frozen_blob(tmp_path, monkeypatch):
    (tmp_path / "docs").mkdir()
    ...
    (tmp_path / "docs/INFINITO_3_V9_RUNNER_MANIFEST.json").write_text(
...
def test_v9_live_mode_requires_exact_revision(monkeypatch):
    ...
    with pytest.raises(SystemExit, match="Exact --expected-revision"):
        runner.main(["--authorize-live-v9", "--expected-revision", "other"])


def test_v9_main_writes_synthetic_evidence_without_real_bank(tmp_path, monkeypatch):
    scenario = TrajectoryScenario(name="synthetic_v9", steps=(TrajectoryStep(
        "Calculate 6 * 7", label="synthetic v9",
        tags=("empty_context",),
        expectation=EvaluationExpectation(answer_contains=("42",)),
    ),))
    monkeypatch.setitem(sys.modules, "src.infinito3.trajectory_holdout_v9_cases", SimpleNamespace(
        independent_trajectory_holdout_v9_suite=lambda: (scenario,)
    ))
    ...
    output = tmp_path / "v9-result"
    assert runner.main([
        "--authorize-live-v9", "--expected-revision", "synthetic-sha",
        "--output-dir", str(output),
    ]) == 0
    report = json.loads((output / "report.json").read_text())
    assert report["metadata"]["evaluation_version"] == "V9"
    assert report["metadata"]["first_v9_execution"] is True
    assert report["metadata"]["post_v8_diagnostic_holdout"] is True
    review = json.loads((output / "audit-review.json").read_text())
    assert review["integration_status"] == "NOT_APPROVED"
    assert review["requires_manual_grounding_review"] is True
    assert review["requires_manual_temporal_canonicalization_review"] is True
