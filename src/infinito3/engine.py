...
class CognitiveEngine:
    ...
    def process(self, text: str, top_k: int = 5, *, context_budget_tokens: int = 1200,
        ...
        transitions = self.temporal_state.apply(events, memory_store=self.memory_store, goal_engine=self.goal_engine) if events else []

        goal_types = {CognitiveEventType.CREATE_GOAL, CognitiveEventType.COMPLETE_GOAL,
        ...
        return CognitiveDecision(input_text=text, safety=safety, gate=gate, stored_memory_id=stored_memory_id,
                                 created_goal_ids=created_goal_ids, cognitive_event_ids=[event.id for event in events],
                                 temporal_transitions=[asdict(t) for t in transitions], context=context,
