...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run frozen held-out INFINITO 3.0 long-horizon trajectories."
    )
...
def main() -> int:
    ...
    report.metadata.update(
        {
            ...
            "suite_sha256": hashlib.sha256(
                (REPO_ROOT / "src" / "infinito3" / "trajectory_holdout_cases.py").read_bytes()
            ).hexdigest(),
        }
    )
    ...
    json_path = output_dir / "infinito3-trajectory-holdout.json"
    markdown_path = output_dir / "infinito3-trajectory-holdout.md"
    json_path.write_text(report.to_json(), encoding="utf-8")
    markdown_path.write_text(report.to_markdown(), encoding="utf-8")

    s = report.summary
    print("INFINITO 3.0 HELD-OUT INDEPENDENT TRAJECTORY EVALUATION")
