...
CONFIG = {
    ...
    "answer_output_floor": 1024, "event_output_floor": 2048, "reranker_output_floor": 512,
    "protocol_revision": "v6_r2_output_budget",
}
...
def preflight():
    manifest = json.loads((ROOT / "docs/INFINITO_3_V6_R2_MANIFEST.json").read_text())
...
class AuditLog:
    ...
    def client(self, client, category, scenario):
        def endpoint(name):
            def create(**payload):
                if name == "responses":
                    floor = (CONFIG["answer_output_floor"] if category.endswith("_answers") else
                             CONFIG["event_output_floor"] if category == "events" else
                             CONFIG["reranker_output_floor"])
                    payload["max_output_tokens"] = max(payload.get("max_output_tokens") or 0, floor)
                ...
                if name == "responses" and raw.get("status") != "completed":
                    length_limited = (raw.get("status") == "incomplete" and
                                      (raw.get("incomplete_details") or {}).get("reason") == "max_output_tokens")
                    if category.endswith("_answers") and length_limited and response.output_text:
                        warning = {"call_id": call_id, "category": category, "scenario": scenario}
                        self.truncated_answers.append(warning)
                        self.write("truncated_answer", **warning)
                    else:
...
def main(argv=None):
    ...
    try:
        ...
        report.metadata.update({"runner_revision": revision, "manifest": manifest, "suite_sha256": SUITE_SHA,
            ...
            "prior_partial_run": 34984319827,
            "independent_first_v6": False,
