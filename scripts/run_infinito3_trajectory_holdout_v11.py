#!/usr/bin/env python3
"""Manual-only V11 runner. Preflight never imports or replays the frozen V11 bank."""
import argparse
import hashlib
...
CASE_COMMIT = "b3942be9029ca884964499acccbb38495ea1e294"
V10_DEVELOPMENT_RUN = 35011969750
CONFIG = {
    **audited.CONFIG,
    "protocol_revision": "v11_r1_lossless_current_planner",
}
...
def preflight():
    manifest = json.loads((ROOT / "docs/INFINITO_3_V11_RUNNER_MANIFEST.json").read_text())
    ...
    suite = ROOT / "src/infinito3/trajectory_holdout_v11_cases.py"
    if _git_blob_sha1(suite) != SUITE_BLOB_SHA1:
        raise SystemExit("Frozen V11 suite mismatch")
    return manifest
...
def audit_report(report):
    ...
    review["evaluation_version"] = "V11"
    review["post_v10_diagnostic_holdout"] = True
    review["requires_manual_planner_review"] = True
    review["requires_manual_literal_fastpath_review"] = True
    review["requires_manual_current_state_review"] = True
...
def main(argv=None):
    ...
    parser.add_argument("--authorize-live-v11", action="store_true")
    parser.add_argument("--expected-revision")
    parser.add_argument("--output-dir", default="trajectory-holdout-v11-results")
    args = parser.parse_args(argv)

    manifest = preflight()
    if not args.authorize_live_v11:
        print("Preflight passed. No V11 turns imported or replayed; no provider calls. Live authorization required.")
        return 0
    ...
    audit.write(
        "run_start",
        evaluation_version="V11",
        revision=revision,
        manifest=manifest,
        case_commit=CASE_COMMIT,
        v10_development_run=V10_DEVELOPMENT_RUN,
    ...
    try:
        ...
        ).run(independent_trajectory_holdout_v11_suite())
        report.metadata.update({
            ...
            "v10_development_run": V10_DEVELOPMENT_RUN,
            "first_v11_execution": True,
            "post_v10_diagnostic_holdout": True,
    ...
    print("V11 completed. Manual semantic/planner/literal/current-state/final-state audit required. No integration authorized.")
    return 0
