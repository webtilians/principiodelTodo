...
1. Dropout reducido: 0.3 → 0.15 (mejor balance regularización/aprendizaje)
2. Hidden dim aumentado: 512 → 640 (más capacidad sin explotar memoria)
...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        """
        # Log softmax para estabilidad numérica
...
class OptimizedTrainer:
    ...
    def __init__(
        ...
        # Optimizer con weight decay más fuerte
    ...
    def get_lr(self):
        ...
        if self.total_steps < self.warmup_steps:
            # Linear warmup
            return self.optimizer.param_groups[0]['lr'] * (self.total_steps / self.warmup_steps)
        else:
            # Cosine decay después de warmup
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (input_ids, labels) in enumerate(self.train_loader):
            ...
            if isinstance(loss_phi, torch.Tensor) and loss_phi.item() > 0:
                loss = loss_lm + self.model.lambda_phi * loss_phi
            else:
                loss = loss_lm
        ...
        if hasattr(self.model, 'learnable_phi_weights') and self.model.learnable_phi_weights is not None:
            # Hacer un forward pass de validación para obtener PHI
            with torch.no_grad():
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for input_ids, labels in self.val_loader:
                ...
                # Solo loss LM en validación (sin PHI)
                loss = self.criterion(
                    logits.reshape(-1, logits.size(-1)),
                    labels.reshape(-1)
                )
    ...
    def train(self, epochs):
        ...
        print(f"\n{'='*80}")
        print(f"ENTRENAMIENTO OPTIMIZADO - {epochs} épocas")
        print(f"{'='*80}")
        ...
        for epoch in range(epochs):
            ...
            if val_loss < self.best_val_loss:
                ...
                print(f"  ⚠️ Sin mejora ({self.epochs_without_improvement}/{self.patience})")
...
def main():
    ...
    if args.resume:
        ...
        print(f"  ✅ Época {checkpoint.get('epoch', 'N/A')}, Val PPL: {checkpoint.get('val_ppl', 'N/A'):.2f}")
